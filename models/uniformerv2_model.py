...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        if temporal_downsample:
            self.conv1 = nn.Conv3d(in_channels, width, (kernel_size, patch_size, patch_size), (2, patch_size, patch_size), (padding, 0, 0), bias=False)
            t_size = t_size // 2
        else:
            self.conv1 = nn.Conv3d(in_channels, width, (1, patch_size, patch_size), (1, patch_size, patch_size), (0, 0, 0), bias=False)
...
if __name__ == '__main__':
    ...
    flops = FlopCountAnalysis(model, torch.rand(1, 4, num_frames, 240, 240))
    s = time.time()
    logger.info(flop_count_table(flops, max_depth=1))
    end = time.time()
    logger.info(f'flops: {flops.total()/1e9}')
    logger.info(f'params: {sum(p.numel() for p in model.parameters() if p.requires_grad)/1e6}')
    logger.info(f'time: {end-s}')
    print(model(torch.rand(1, 4, num_frames, 240, 240)).shape)
    print(f'number of parameters: {sum(p.numel() for p in model.parameters() if p.requires_grad)}')
    print(f'flops: {flops.total()/1e9}')
    print(f'params: {sum(p.numel() for p in model.parameters() if p.requires_grad)/1e6}')
    print(f'time: {end-s}')
