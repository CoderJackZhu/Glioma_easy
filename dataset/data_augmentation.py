...
def random_augmentation(arr_img, arr_mask, shift_range, scale_range, gamma_range, p):
    if random.random() < p:
        arr_img = random_gamma_transformation(arr_img, gamma_range, p)
        arr_img, arr_mask = random_flip(arr_img, arr_mask, p)
        arr_img, arr_mask = random_permute(arr_img, arr_mask, p)
        arr_img, arr_mask = random_rotate(arr_img, arr_mask, 3, p)
        arr_img, arr_mask = random_shift(arr_img, arr_mask, shift_range, p)
        arr_img, arr_mask = random_scale(arr_img, arr_mask, scale_range, p)
        arr_img = random_noise(arr_img, p)
    return arr_img, arr_mask


def patch_random_augmentation(arr_img, arr_mask, shift_range, scale_range, gamma_range, p):
    if random.random() < p:
...
def random_shift(arr_img, arr_mask, shift_range, p):
    if random.random() < p:
...
def random_flip(arr_img, arr_mask, p):
    if random.random() < p:
...
def random_scale(arr_image, arr_mask, scale_factor_range, p):
    if random.random() < p:
